...
rows = [("prod", r"$\mathrm{TurboQuant}_{\mathrm{prod}}$"),
        ("mse",   r"$\mathrm{TurboQuant}_{\mathrm{mse}}$")]
bits_list = [1, 2, 3, 4]
...
for i, (qtype, label) in enumerate(rows):
    for j, b in enumerate(bits_list):
        ax = axes[i, j]
        err = pd.read_csv(f"./ip_error_dim1536_bw{b}_{qtype}.csv")['ip_error'].values.flatten()
...
plt.savefig("turbo_ip_err.png", bbox_inches='tight', dpi=300)